(function () {
  ...
  function policyTags(key) {
    const tags = [];
    if (key.knock_required) tags.push('<span class="tag knock">SPA required</span>');
    if (key.allow_from) tags.push('<span class="tag">Source restricted</span>');
    if (key.expires) tags.push('<span class="tag warn">Expires ' + escapeText(key.expires) + '</span>');
    if (!key.port_forwarding) tags.push('<span class="tag">Tunneling disabled</span>');
    if (!key.agent_forwarding) tags.push('<span class="tag">Agent disabled</span>');
    if (!key.enabled) tags.push('<span class="tag warn">Disabled</span>');
    return tags.join("");
  }
  ...
  function render() {
    ...
    keys.forEach(key => {
      ...
      article.innerHTML =
        ...
        '<div class="key-main"><div class="key-title"><strong>' + escapeText(key.comment || "Unnamed public key") + '</strong><span class="badge ' + (key.enabled ? "active" : "inactive") + '">' + (key.enabled ? "Enabled" : "Disabled") + '</span></div>' +
        ...
        '<button class="edit-button" type="button">Security Settings</button>';
  ...
  function setUsers() {
    ...
    filter.replaceChildren(new Option("All accounts", ""), ...state.users.map(user => new Option(user.name, user.name)));
    filter.value = prior;
  ...
  function setKnockStatus() {
    ...
    badge.textContent = good ? "Running" : state.knock.installed ? "Needs attention" : "Not installed";
    card.classList.toggle("warning", !good);
    let text = state.knock.installed ? "fwknop is installed" : "fwknop was not detected";
    text += state.knock.active ? " and the service is running" : ", but the service is stopped";
    text += state.knock.access_configured ? "; an SPA access policy is configured." : "; no valid access policy was detected.";
  ...
  function openSettings(key) {
    state.selected = key;
    document.getElementById("settings-title").textContent = key.comment || "Unnamed public key";
    ...
    document.getElementById("form-error").textContent = key.command_conflict ? "This public key has a custom forced command. Remove it before enabling SPA-gated access." : "";
  ...
  async function submitSettings(event) {
    ...
    try {
      ...
      toast("Public-key security settings saved");
      await refresh();
    } catch (caught) {
  ...
  async function deleteKey() {
    if (!state.selected || !window.confirm("Permanently delete “" + (state.selected.comment || state.selected.fingerprint) + "”? This immediately revokes SSH login access.")) return;
  ...
  async function showLogs() {
    ...
    } catch (error) { output.textContent = "Read failed: " + (error.message || error); }
  }
